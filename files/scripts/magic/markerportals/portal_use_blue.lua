...
function collision_trigger( player_id )
    ...
    local pos_x = tonumber( GlobalsGetValue( "mocreep_markerportal_blue_x", "0" ) )
    local pos_y = tonumber( GlobalsGetValue( "mocreep_markerportal_blue_y", "0" ) )

    if pos_x == 0 and pos_y == 0 then
        GamePrint("You feel the portal lacks focus")
	else
	    EntitySetTransform( player_id, pos_x, pos_y )
        EntityLoad("mods/mo_creeps/files/entities/projectiles/deck/markerportals/portal_blue_fx.xml", pos_x, pos_y)
    end
