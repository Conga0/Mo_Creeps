...
if found then
    ...
    if symbolinitiated == false then
		EntityLoad( "mods/mo_creeps/files/entities/items/books/book_symbol_" .. symbolName .. ".xml", x, y )
        EntityLoad("mods/mo_creeps/files/entities/particles/poof_green_dense.xml", x, y )
        ComponentSetValue2( comp, "value_bool", true )
    end
    if musicinitiated ~= true then
        GlobalsSetValue( "mocreep_king_secret_music", "1" )
